...
import checkers.util.CheckersSQLUtil;
...
public class GameBoard {
    ...
    //Use a username to enter the game
    public void enterUser(String userName){
        User userQuery = CheckersSQLUtil.getInstance().queryUserByName(userName);
        if(userQuery != null){
            this.currentUser = userQuery;
        }else {
            CheckersSQLUtil.getInstance().addUserByName(userName);
            this.currentUser = CheckersSQLUtil.getInstance().queryUserByName(userName);
        }
    }

    //Save game record
    public boolean saveGameRecord(String gameName){
        GameRecord gameRecord = CheckersSQLUtil.getInstance().queryGameRecord(gameName, currentUser.getName());

        if(gameRecord != null){
            return false;//The game record name already exists
        }else {
            CheckersSQLUtil.getInstance().addGame(gameName, currentUser.getId(), serialise());
            return true;
        }
    }

    //Update an existing game record
    public boolean updateGameRecord(String gameName){
        GameRecord gameRecord = CheckersSQLUtil.getInstance().queryGameRecord(gameName, currentUser.getName());

        if(gameRecord != null){
            CheckersSQLUtil.getInstance().updateGame(gameName, currentUser.getId(), serialise());
            return true;
        }
        return false;
    ...
    //Load game record
    public void loadGameRecord(String gameName){
        GameRecord gameLoad = CheckersSQLUtil.getInstance().queryGameRecord(gameName, currentUser.getName());
        if(gameLoad != null){
            deserialise(gameLoad.getSerialization());
        }
    }

    //Show all available game records of current user
    public List<String> getAvailableRecords(){
        List<String> records = new ArrayList<>();
        List<GameRecord> gameRecords = CheckersSQLUtil.getInstance().queryGamesByUser(currentUser.getId());
        for(GameRecord r: gameRecords){
            records.add(r.getGameName());
        }
